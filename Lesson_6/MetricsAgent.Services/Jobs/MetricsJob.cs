...
#pragma warning disable CA1416 // Проверка совместимости платформы
	public class MetricsJob : IJob, IDisposable
	{
		...
		public MetricsJob(IServiceProvider scopeFactory)
		{
			scope = scopeFactory.CreateScope();
			_cpuRepo = scope.ServiceProvider.GetRequiredService<IDbRepository<CpuMetric>>();
			_ramRepo = scope.ServiceProvider.GetRequiredService<IDbRepository<RamMetric>>();
			_hddRepo = scope.ServiceProvider.GetRequiredService<IDbRepository<HddMetric>>();
			_dotnetRepo = scope.ServiceProvider.GetRequiredService<IDbRepository<DotNetMetric>>();
			_networkRepo = scope.ServiceProvider.GetRequiredService<IDbRepository<NetworkMetric>>();
